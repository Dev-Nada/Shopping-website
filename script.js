      ...
      <ul>
        ...
        <li class="drop">
          ...
          <ul class="drop-content">
            <li><a href="generalcat.html">Dresses & Acc</a></li>
            <li><a href="makeup.html">Makeup</a></li>
            <li><a href="shoes.html">Shoes & Bags</a></li>
          </ul>
        </li>
        <li><a href="cart.html">Basket</a> <span class="num-cart"></span></li>
...
function addToCart(event) {
  ...
  const card = event.target.closest(".product") || event.target.closest(".best-item");
  if (!card) return;

  // Find the title and price (handles both class names and plain p tags)
  let titleEl = card.querySelector(".title") || card.querySelector("p:first-of-type");
  let priceEl = card.querySelector(".price") || card.querySelector("p:nth-of-type(2)");
  ...
  if (confirm("Item added! Do you want to go to the cart now? \n(Click OK for Cart, Cancel to keep shopping)")) {
...
function validateCheckout(event) {
  ...
  let isValid = true;
  const inputs = document.querySelectorAll('#main-checkout-form input[required]');
  ...
  if (isValid) {
      alert("Order placed successfully! Thank you for shopping with Lady Boutique.");
